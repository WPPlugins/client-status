...
<div class="wrap">
	...
	switch($action){
		case "update":
			if(isset($_POST['security_key'])){
				if($this->options->security_key != $_POST['security_key']){
					$old_hash = $this->options->security_key;
					$this->options->security_key = $_POST['security_key'];
					$this->update_encryption($old_hash);	// Keep client username/password hashes in sync
				}
			}
			if(isset($_POST['install_type'])){
				$this->options->install_type = $_POST['install_type'];
			}
			if($this->options->install_type == CLIENT_STATUS_INSTALL_TYPE_DASHBOARD){
				...
				if(isset($_POST['allow_cron_updates'])){
					$this->options->allow_cron_updates = $_POST['allow_cron_updates'];
				} else {
					$this->options->allow_cron_updates = 0;
				...
				if(isset($_POST['admin_emails'])){					
					$this->options->admin_emails = $_POST['admin_emails'];
				}
				
				if(isset($_POST['expand_client_info'])){
					$this->options->expand_client_info = $_POST['expand_client_info'];
				} else {
					$this->options->expand_client_info = 0;
				}
				
				if(isset($_POST['show_quick_info'])){
					$this->options->show_quick_info = $_POST['show_quick_info'];
				} else {
					$this->options->show_quick_info = 0;
				}
			}
	...
	?>
		...
		<tr valign="top">
			...
			<td>
				<select id="install_type" name="install_type">
					<option value="<?php echo CLIENT_STATUS_INSTALL_TYPE_CLIENT; ?>"<?php echo (($this->options->install_type == CLIENT_STATUS_INSTALL_TYPE_CLIENT) ? ' selected="selected"' : '')?>><?php _e("Client"); ?></option>
					<option value="<?php echo CLIENT_STATUS_INSTALL_TYPE_DASHBOARD; ?>"<?php echo (($this->options->install_type == CLIENT_STATUS_INSTALL_TYPE_DASHBOARD) ? ' selected="selected"' : '')?>><?php _e("Dashboard"); ?></option>
				</select>
			</td>
		</tr>
		<tr valign="top">
			...
			<td>
				<input <?php echo (($this->options->install_type == CLIENT_STATUS_INSTALL_TYPE_DASHBOARD) ? 'type="text"' : 'type="password"'); ?> name="security_key" id="security_key" value="<?php echo $this->options->security_key; ?>" />
			</td>
		</tr>
		<tr valign="top" class="client_status_dashboard_only">
			...
			<td>
				<input type="checkbox" name="allow_cron_updates" id="allow_cron_updates" value="1"<?php echo (($this->options->allow_cron_updates == 1) ? ' checked="checked"' : '')?> /><label for="allow_cron_updates"><?php _e('Allow Client Updates'); ?></label>
				<select name="update_frequency" id="update_frequency">
					<option value="<?php echo CLIENT_STATUS_CRON_HOURLY; ?>"<?php echo (($this->options->update_frequency == CLIENT_STATUS_CRON_HOURLY) ? ' selected="selected"' : '')?>><?php _e('Hourly'); ?></option>
					<option value="<?php echo CLIENT_STATUS_CRON_TWICE_DAILY; ?>"<?php echo (($this->options->update_frequency == CLIENT_STATUS_CRON_TWICE_DAILY) ? ' selected="selected"' : '')?>><?php _e('Twice Daily'); ?></option>
					<option value="<?php echo CLIENT_STATUS_CRON_DAILY; ?>"<?php echo (($this->options->update_frequency == CLIENT_STATUS_CRON_DAILY) ? ' selected="selected"' : '')?>><?php _e('Daily'); ?></option>
				</select>
			</td>
		</tr>
		<tr valign="top" class="client_status_dashboard_only">
			...
			<td>
				<select name="admin_emails[]" multiple="multiple" size="5" style="height: 80px;">
					...
					?>
					<option value="<?php echo $user->ID; ?>"<?php echo ((in_array($user->ID, $this->options->admin_emails)) ? ' selected="selected"': ''); ?>><?php echo $user->display_name; ?></option>
					<?php
							}
						}
					?>
		...
		<tr valign="top" class="client_status_dashboard_only">
			...
			<td>
				<select name="expand_client_info" id="expand_client_info">
					<option value="1"<?php echo (($this->options->expand_client_info == 1) ? ' selected="selected"' : '')?>><?php _e('Yes'); ?></option>
					<option value="0"<?php echo (($this->options->expand_client_info == 0) ? ' selected="selected"' : '')?>><?php _e('No'); ?></option>
				</select>
			</td>
		</tr>
		<tr valign="top" class="client_status_dashboard_only">
			...
			<td>
				<select name="show_quick_info" id="show_quick_info">
					<option value="1"<?php echo (($this->options->show_quick_info == 1) ? ' selected="selected"' : '')?>><?php _e('Yes'); ?></option>
					<option value="0"<?php echo (($this->options->show_quick_info == 0) ? ' selected="selected"' : '')?>><?php _e('No'); ?></option>
				</select>
			</td>
		</tr>
		</table>
