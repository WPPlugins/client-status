...
global $client_status, $wpdb;
if(isset($_REQUEST['security_key']) && $_REQUEST['security_key'] != ''){
	if(md5($client_status->options->security_key) == $_REQUEST['security_key']){
		...
		echo "<debug_mode>" . (defined('WP_DEBUG') && WP_DEBUG ? "1" : "0") . "</debug_mode>\r\n";
		...
		echo "<enable_xmlrpc>" . get_option('enable_xmlrpc') . "</enable_xmlrpc>\r\n";
		echo "<posts>\r\n";
		...
		if(count($theme_updates) > 0){
			...
			foreach($theme_updates as $key=>$theme){
				...
				echo "<author_name><![CDATA[" . $theme->author . "]]></author_name>\r\n";
				echo "<author_uri><![CDATA[" . $theme->authorURI . "]]></author_uri>\r\n";
				...
				echo "<parent_theme>" . $theme->template . "</parent_theme>\r\n";
